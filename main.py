...
# --- 3. Business Logic: Elements (ฟังก์ชันที่หายไป) ---
...
def create_full_pdf(comp_info, basket, curr_info, date_str, time_str, element_text):
    ...
    # Barcode (ฟังก์ชันที่หายไป)
    try:
        bc_class = barcode.get_barcode_class('code128')
        bc_obj = bc_class("SSP2TJJ1PQB4", writer=ImageWriter())
        bc_buffer = BytesIO()
        bc_obj.write(bc_buffer)
        c.drawImage(bc_buffer, (w-55*mm)/2, curr_y-12*mm, width=55*mm, height=12*mm)
        curr_y -= 15*mm
    except: pass
    ...
    # QR Code (ฟังก์ชันที่หายไป)
    curr_y -= 30*mm
    qr = qrcode.make("https://maps.google.com"); qr_buffer = BytesIO()
    qr.save(qr_buffer, format='PNG'); qr_buffer.seek(0)
    c.drawImage(qr_buffer, 8*mm, curr_y, width=20*mm, height=20*mm)
    c.setFont(FONT_NAME, 7)
...
with col_in:
    ...
    with st.form("add_item"):
        ...
        q, p, d = st.columns(3)
        qty = q.number_input("จำนวน", min_value=1)
        prc = p.number_input("ราคาต่อหน่วย")
        dsc = d.number_input("ส่วนลด (Discount)")
...
with col_pre:
    ...
    if st.session_state.basket:
        ...
        # แสดงรายการในตารางเพื่อให้ดูง่าย (แทน Preview PDF ที่มักโดนบล็อก)
        ...
        if st.button("💾 บันทึกลงประวัติ"):
            ...
            st.session_state.history.append({"วันที่": in_date.strftime("%Y-%m-%d"), "บริษัท": sel_comp, "ยอดรวม": total})
